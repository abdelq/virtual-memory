...
/* Recherche dans la table des pages et renvoie le `frame_number` si valide,
...
/* Modifie l'entrée de `page_number` dans la page table pour qu'elle pointe
 * vers `frame_number` */
...
bool pt_readonly_p(unsigned int page_number)
{
	...
	if (page_table[page_number].valid)	// XXX Necessary?
